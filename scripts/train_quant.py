...
def train(bitspec_name: str, model_name: str, loss_name: str, dname: str, output_dir: str, num_epochs: int, model_type: torch.dtype, restart_epoch: int = -1):
    """
    ...
    hot_start_model, bs = build_model(model_name, input_dim, hidden_dim, output_dim, u_max, bitspec_name)

    if model_name == "BM":
        model = QuantBoundedModel(input_dim, hidden_dim, output_dim, u_max, bs)
    else:
        raise "No quantization model defined for: " + model_name

    loss_fn = build_loss(loss_name)

    output_dir = output_dir + "_" + model.my_name() + "_" + loss_fn.my_name() + "_L" + str(model.my_linear_ops()) + "_" + bitspec_name
    ...
    if restart_epoch > 0:
        ...
        hot_start_model.load_state_dict(ckpt["model_state"])
        #optimizer.load_state_dict(ckpt["optimizer_state"])
        # transfer weights to the quant model
        model.from_bounded_model(hot_start_model)
    ...
    # pump some batches through to get some stats for
    # quantification
    model.train()
    with torch.no_grad():
        i_batch = 0
        for xb, yb in train_loader:
            xb = xb.to(device)
            _ = model(xb)
            if i_batch >= 20:  # ~10–50 batches is usually enough
                break
            i_batch += 1
